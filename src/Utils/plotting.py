...
def plot_cv_indices(cv_split_generator, n_samples, n_splits):
    """
    Crea un grafico che visualizza i fold della Time Series Cross Validation.
    """
    ...
    for ii, (tr, tt) in enumerate(splits):
        ...
        ax.scatter(tr, [ii + .5] * len(tr),
                   color=c_train, marker='_', lw=10)

        # --- Disegna Validation (Rosso) ---
        ax.scatter(tt, [ii + .5] * len(tt),
                   color=c_val, marker='_', lw=10)

    # Formattazione Grafico
    yticklabels = [f"Fold {i+1}" for i in range(n_splits)]
    ax.set(yticks=np.arange(n_splits) + .5, yticklabels=yticklabels,
           xlabel='Indice Temporale (Ore)', ylabel="Iterazione",
           ylim=[n_splits+0.2, -.2], xlim=[0, n_samples])
