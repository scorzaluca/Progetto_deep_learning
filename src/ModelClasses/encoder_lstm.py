...
from ..config import LOOKBACK, HORIZON, INPUT_SIZE, TARGET_IDX, PATCHTST_CONFIG
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, model_config: dict):
        ...
        if self.pretrain_path:
            ...
            self.encoder.model.model.encoder.load_state_dict(encoder_state, strict=False)
        ...
        # 3. Head per la previsione
        self.head = nn.Linear(self.lstm_hidden, HORIZON)
        
        print(f"EncoderLSTM - d_model: {self.d_model}, "
    ...
    def forward(self, x):
        ...
        """
        # 1. Estrai embeddings dall'encoder (congelato)
        embeddings = self.encoder.get_embeddings(x)  # (batch, n_patches, d_model)
        
        # 2. LSTM processa la sequenza di patch
        lstm_out, _ = self.lstm(embeddings)  # (batch, n_patches, lstm_hidden)
        
        # 3. Usa l'ultimo output per la previsione
        last_out = lstm_out[:, -1, :]  # (batch, lstm_hidden)
        
        # 4. Proietta su horizon
