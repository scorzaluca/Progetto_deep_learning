...
class LSTM(nn.Module):
    ...
    def __init__(self, model_config: dict, train_loader):
        ...
        print(
            f"LSTM - Features: {self.input_size}, Hidden: {self.hidden_size}, Layers: {self.num_layers}"
        )

        # NOTA: bidirectional=False è intenzionale per forecasting
        # In previsione non possiamo usare info future, quindi la direzione
        # backward creerebbe data leakage che non generalizza su dati reali.
        #
        ...
        self.lstm = nn.LSTM(
            ...
            bidirectional=False,  # Forzato per forecasting (no data leakage)
