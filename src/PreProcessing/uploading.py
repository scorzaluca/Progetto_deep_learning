...
class Uploader:
    ...
    def merge_dataset(self, wx_path, pv_path, output_path_csv, output_path_excel):
        ...
        print(f"Colonne trovate nel WX: {df_wx.columns.tolist()}")
        df_list = [df_wx, df_pv]
        df_merged = pd.concat(df_list, axis=1)
        df_merged.drop(
            columns=["datetime"], inplace=True
        )  # levo quella del df_pv perchè aveva gli orari non
        ...
        )  # Sydney (erano misti +10/+11)
        df_merged["dt_iso"] = df_merged["dt_iso"].dt.tz_convert(
            tz_fixed
        )  # tutti i record con lo stesso fuso
        # orario +10 (c'era l'ora legale +11)

        # Salva CSV (supporta timezone-aware)
        df_merged.to_csv(output_path_csv, index=False)
        print(f"Dataset completo (merged) formato csv salvato in {output_path_csv}")

        # Per Excel: rimuoviamo timezone info (Excel non supporta tz-aware datetime)
        df_merged["dt_iso"] = df_merged["dt_iso"].dt.tz_localize(None)
        df_merged.to_excel(output_path_excel, index=False)
        print(f"Dataset completo (merged) formato xlsx salvato in {output_path_excel}")
        return df_merged
